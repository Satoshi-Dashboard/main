...
export async function cacheGetJsonBatch(keys) {
  const results = {};
  const remotePending = [];
  ...
  for (const key of keys) {
    ...
    const local = getLocalCache(namespaced);
    if (typeof local === 'string') {
      try { results[key] = JSON.parse(local); } catch { /* skip corrupt */ }
    } else {
      remotePending.push(key);
    }
  }

  if (REMOTE_ENABLED && remotePending.length > 0) {
    const namespacedKeys = remotePending.map((k) => withPrefix(k));
    const res = await remoteCommand(['MGET', ...namespacedKeys]);
    if (res.ok && Array.isArray(res.result)) {
      res.result.forEach((val, i) => {
        if (typeof val === 'string') {
          try { results[remotePending[i]] = JSON.parse(val); } catch { /* skip */ }
        }
      });
    }
  }
