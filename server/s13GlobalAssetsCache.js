...
class S13GlobalAssetsError extends Error {
  constructor(message) {
    super(message);
    this.name = 'S13GlobalAssetsError';
  }
}
...
function parseSnapshotFromMarkdown(text) {
  ...
  if (titleIndex < 0) {
    throw new S13GlobalAssetsError('Newhedge snapshot section missing');
  }
  ...
  if (!rows.length) {
    throw new S13GlobalAssetsError('Newhedge snapshot rows missing');
  }
  ...
  if (!assets.length) {
    throw new S13GlobalAssetsError('Newhedge snapshot assets missing');
  }
...
async function fetchTextWithTimeout(url) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new S13GlobalAssetsError(`Newhedge scrape HTTP ${response.status}`);
    }

    const text = await response.text();
    if (!text || text.length < 200) {
      throw new S13GlobalAssetsError('Newhedge scrape returned empty content');
    }

    return text;
  } catch (error) {
    if (error?.name === 'AbortError') {
      throw new S13GlobalAssetsError('Newhedge scrape request timeout');
    }
    if (error instanceof S13GlobalAssetsError) throw error;
    throw new S13GlobalAssetsError(error instanceof Error ? error.message : String(error));
  } finally {
...
export async function getS13GlobalAssetsPayload({ forceFresh = false } = {}) {
  ...
  const refreshed = await withCacheLock(
    SHARED_LOCK_KEY,
    async () => refreshS13GlobalAssetsPayload(),
...
export async function getS13GlobalAssetsStatus() {
