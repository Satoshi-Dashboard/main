...
import { getBootstrapSnapshot } from './features/bootstrap/bootstrapSnapshot.js';
...
export function createApp() {
  ...
  // ── Bootstrap: single-request snapshot of all module caches ──
  app.get('/api/bootstrap', asyncRoute(async (_req, res) => {
    setDataCacheHeaders(res, { sMaxAge: 15, swr: 60 });
    const snapshot = await getBootstrapSnapshot();
    res.json(snapshot);
  }));

  // ── Cron warmup: Vercel Hobby daily refresh for slow-changing data ──
  app.get('/api/cron/warmup', asyncRoute(async (req, res) => {
    ...
    const tasks = [
      { label: 'btcRates', fn: () => getBtcRates() },
      { label: 'mempoolOverview', fn: () => getMempoolOverviewPayload() },
      { label: 'fearGreed', fn: () => getFearGreedPayload({ limit: 31 }) },
      { label: 's03MultiCurrency', fn: () => getS03MultiCurrencyPayload() },
      { label: 's10StablecoinList', fn: () => getS10StablecoinList() },
      { label: 's10StablecoinPrices', fn: () => getS10StablecoinLivePrices() },
      { label: 'bitnodes', fn: () => getBitnodesPayload() },
      { label: 'lightningWorld', fn: () => getLightningWorldPayload() },
      { label: 's12BtcDistribution', fn: () => getS12BtcDistributionPayload() },
      { label: 's13AddressesRicher', fn: () => getS13AddressesRicherPayload() },
      { label: 's14GlobalAssets', fn: () => getS14GlobalAssetsPayload() },
      { label: 's15BtcVsGold', fn: () => getS15BtcVsGoldMarketCapPayload() },
      { label: 's30UsDebt', fn: () => getUsNationalDebtPayload() },
      { label: 'btcmapBusinesses', fn: () => getBtcMapBusinessesByCountryPayload() },
      { label: 'binanceHistory365', fn: () => getBinanceBtcHistoryPayload({ days: 365, interval: '1d' }) },
    ];
    ...
    res.json({
      refreshed: summary.filter((s) => s.ok).length,
