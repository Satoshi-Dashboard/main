...
const JOHOE_BTC_QUEUE_RANGE_CONFIG = {
  '24h': {
  ...
  '30d': {
    historyFeedKey: 'johoeBtcQueueHistory30d',
    cacheKey: 'public:johoe:btc-queue:30d:composed',
    lockKey: 'public:johoe:btc-queue:30d:composed:refresh',
    fallbackTtlSeconds: 300,
  },
  all: {
    historyFeedKey: 'johoeBtcQueueHistoryAll',
    cacheKey: 'public:johoe:btc-queue:all:composed',
    lockKey: 'public:johoe:btc-queue:all:composed:refresh',
    fallbackTtlSeconds: 900,
  },
};
const JOHOE_BTC_QUEUE_BOOTSTRAP_POINT_BUDGET = {
  '24h': 180,
  '30d': 144,
  all: 96,
};
...
const FEED_DEFS = {
  ...
  johoeBtcQueueHistory30d: {
    cacheKey: 'public:johoe:btc-queue:history:30d',
    lockKey: 'public:johoe:btc-queue:history:30d:refresh',
    refreshMs: 900_000,
    sourceProvider: 'supabase edge + supabase postgres',
    sourceUrl: `${SUPABASE_EDGE_FUNCTIONS_BASE_URL}/${JOHOE_BTC_QUEUE_HISTORY_FUNCTION}?range=30d`,
    safeMinuteBudget: 1,
    safeDailyBudget: 96,
  },
  johoeBtcQueueHistoryAll: {
    cacheKey: 'public:johoe:btc-queue:history:all',
    lockKey: 'public:johoe:btc-queue:history:all:refresh',
    refreshMs: 21_600_000,
    sourceProvider: 'supabase edge + supabase postgres',
    sourceUrl: `${SUPABASE_EDGE_FUNCTIONS_BASE_URL}/${JOHOE_BTC_QUEUE_HISTORY_FUNCTION}?range=all`,
    safeMinuteBudget: 1,
    safeDailyBudget: 4,
  },
...
function isValidJohoeRange(value) {
  return value === '24h' || value === '30d' || value === 'all';
}

function normalizeJohoeRange(value) {
  const range = String(value || '24h').toLowerCase();
  return isValidJohoeRange(range) ? range : '24h';
}
...
async function getJohoeBtcQueueHistoryRawPayload(range = '24h') {
  const normalizedRange = normalizeJohoeRange(range);
  const config = JOHOE_BTC_QUEUE_RANGE_CONFIG[normalizedRange];
  const params = new URLSearchParams({ range: normalizedRange, includeBuckets: 'true' });

  return getFeed(
    config.historyFeedKey,
    async () => {
      const raw = await fetchSupabaseEdgeFunctionJson(JOHOE_BTC_QUEUE_HISTORY_FUNCTION, {
        searchParams: { range: normalizedRange },
        timeoutMs: 30_000,
      }).catch(() => fetchJsonWithTimeout(`${SCRAPER_BASE_URL}${JOHOE_BTC_QUEUE_HISTORY_PATH}?${params.toString()}`, { timeoutMs: 30_000 }));
      ...
      return normalizeJohoeHistoryResponse(raw, normalizedRange);
    },
  ...
  return getFeed(
    ...
    async () => {
      const raw = await fetchSupabaseEdgeFunctionJson(JOHOE_BTC_QUEUE_LATEST_FUNCTION, {
        timeoutMs: 15_000,
      }).catch(() => fetchJsonWithTimeout(`${SCRAPER_BASE_URL}${JOHOE_BTC_QUEUE_LATEST_PATH}`, { timeoutMs: 15_000 }));

      if (!validateJohoeLatestPayload(raw)) {
        throw new PublicFeedError('Johoe BTC queue latest payload is incomplete');
      }
      return normalizeJohoeLatestResponse(raw);
    },
    (value) => validateObject(value) && Number.isFinite(Number(value?.ts)),
  );
}

export async function getJohoeBtcQueuePayload({ range = '24h' } = {}) {
  const normalizedRange = normalizeJohoeRange(range);
  const config = JOHOE_BTC_QUEUE_RANGE_CONFIG[normalizedRange];
  ...
  return getComposedPayload({
    ...
    fallbackTtlSeconds: config.fallbackTtlSeconds,
    ...
    buildPayload: async () => {
      ...
      return {
        ...
        next_update_at: nextUpdateCandidates.length
          ...
          : null,
        source_provider: Array.from(new Set([historyPayload?.source_provider, latestPayload?.source_provider].filter(Boolean))).join(' | ') || 'supabase edge + supabase postgres',
...
export async function getJohoeBtcQueueBootstrapPayload({ range = '24h' } = {}) {
  const normalizedRange = normalizeJohoeRange(range);
  const payload = await getJohoeBtcQueuePayload({ range: normalizedRange });
  const fullData = payload?.data;
  const previewBudget = JOHOE_BTC_QUEUE_BOOTSTRAP_POINT_BUDGET[normalizedRange] || JOHOE_BTC_QUEUE_BOOTSTRAP_POINT_BUDGET['24h'];
