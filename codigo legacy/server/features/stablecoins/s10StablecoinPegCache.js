...
async function fetchJsonWithTimeout(url) {
  ...
  try {
    ...
    if (!res.ok) {
      throw new S10StablecoinError(`${provider} HTTP ${res.status}`);
    }

    return await res.json();
  } catch (error) {
    if (error?.name === 'AbortError') {
      throw new S10StablecoinError(`${provider} request timeout`);
    }
    if (error instanceof S10StablecoinError) throw error;
    throw new S10StablecoinError(error instanceof Error ? error.message : String(error));
  } finally {
...
function buildLivePricePayload(listPayload) {
  ...
  if (Object.keys(pricesBySymbol).length === 0) {
    throw new S10StablecoinError('Invalid live stablecoin price payload');
  }
  ...
  return {
    ...
    refresh_policy: {
      ...
      safe_daily_budget: null,
      safe_monthly_budget: Math.floor(COINGECKO_HARD_MONTHLY_LIMIT * 0.8),
      derived_from: '/api/s10/stablecoins',
...
async function refreshList() {
  ...
  if (!validateListPayload(data)) {
    throw new S10StablecoinError('Invalid CoinGecko stablecoins payload');
  }
...
async function refreshLivePrices() {
  const listPayload = await getS10StablecoinList();
...
export async function getS10StablecoinList() {
...
export async function getS10StablecoinLivePrices() {
