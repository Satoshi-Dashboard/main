...
import { fetchMempoolOverviewBundle, resolveMempoolOverviewBundle } from '@/shared/services/mempoolApi.js';
...
function Tile({ label, value, variant = 'number', decimals, suffix, accent, source }) {
  return (
    <div className="flex min-h-[108px] flex-col items-center justify-center bg-[#111111] px-3 py-2 select-none sm:px-4 sm:py-3">
      <div className="flex min-h-[2.4em] w-full max-w-full items-center justify-center" style={{ fontSize: 'var(--fs-hero)' }}>
        {value == null ? (
          <div className="skeleton w-3/4 max-w-[18rem]" style={{ height: '2.4em' }} />
        ) : (
          <div className="flex max-w-full flex-wrap items-end justify-center gap-x-1 gap-y-1 text-center font-mono font-bold leading-[1] text-white tabular-nums">
      ...
      <div className="mt-1 text-center uppercase tracking-[0.14em] text-white/50" style={{ fontSize: 'var(--fs-tag)', visibility: source ? 'visible' : 'hidden' }}>
        {source ? `src: ${source}` : '\u00A0'}
      </div>
    </div>
  );
...
function FearGreedTile({ value, classification, history }) {
  ...
  return (
    <div className="visual-chart-surface flex min-h-[108px] flex-col items-center justify-center gap-0.5 bg-[#111111] px-3 py-2 select-none sm:px-4">
      <div className="flex min-h-[1em] items-center justify-center" style={{ fontSize: 'var(--fs-hero)' }}>
        {loading ? (
          <div className="skeleton w-3/4 max-w-[14rem]" style={{ height: '1em' }} />
        ) : (
          <AnimatedMetric
            value={value}
      ...
      {loading ? (
        <div className="skeleton w-1/2" style={{ height: '1em', fontSize: 'var(--fs-caption)' }} />
      ) : (
        <div
...
function DifficultyTile({ pct, etaBlocks, changeNext, changePrev }) {
  ...
  return (
    <div className="visual-svg-surface flex min-h-[160px] flex-col items-center justify-center gap-2 bg-[#111111] px-3 py-3 select-none sm:min-h-[108px] sm:px-4 sm:py-3">
      ...
      <div className="hidden min-h-[1.4em] sm:flex sm:items-center sm:justify-center" style={{ fontSize: 'var(--fs-title)' }}>
        {loading ? (
          <div className="skeleton" style={{ width: '5em', height: '1.2em' }} />
        ) : (
          <AnimatedMetric
            value={pct}
      ...
      <div className="flex min-h-[1.2em] max-w-full flex-wrap items-center justify-center gap-x-1 text-center font-mono" style={{ fontSize: 'var(--fs-micro)', color: UI_COLORS.textTertiary }}>
        {loading ? (
          <div className="skeleton" style={{ width: '10em', height: '1em' }} />
        ) : (
...
export default function S01_BitcoinOverview() {
  ...
  const { data: spotData } = useModuleData(fetchSpot, {
    ...
    bootstrapKey: 'btcRates',
    // Redis guarda { btc_usd, btc_change_24h_pct, source_btc, ... }
    // fetchBtcSpot() devuelve { usd, change24h, source, ... }
    bootstrapTransform: (raw) => {
      const usd = Number(raw?.btc_usd);
      if (!Number.isFinite(usd) || usd <= 0) return null;
      const change24h = Number(raw?.btc_change_24h_pct ?? 0);
      const sourceBtc = String(raw?.source_btc || '').toLowerCase();
      return {
        usd,
        change24h: Number.isFinite(change24h) ? change24h : 0,
        marketCap: usd * 19_900_000,
        supply: null,
        source: sourceBtc.includes('coingecko') ? 'coingecko_fallback' : 'binance',
      };
    },
  });
  ...
  const { data: overviewData } = useModuleData(fetchOverview, {
    ...
    initialData: null,
    keepPreviousOnError: true,
    bootstrapKey: 'mempoolOverview',
    // Redis guarda { data: { difficulty, fees, hashrate, mempool, block_height, fear_greed }, ... }
    // fetchMempoolOverviewBundle() devuelve { overview, mempool, fees }
    bootstrapTransform: (raw) => raw?.data ? resolveMempoolOverviewBundle(raw, null) : null,
  });
