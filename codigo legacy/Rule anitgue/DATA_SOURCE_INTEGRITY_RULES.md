...
| S17 Price Performance | US Median Home Price in BTC | BTC spot from Binance; U.S. median home price time series from FRED MSPUS via `/api/s17/house-price` backend endpoint | Quarterly FRED data on load; BTC spot on load | Backend serves cached FRED+Binance comparison payload; stale cache on failure | Do not replace FRED or Binance silently |
| S18 Cycle Spiral | Live interactive halving-cycle spiral | Local handcrafted halving cycle waypoint data; fully interactive SVG with tooltips, phase legend, responsive scaling | Static (waypoints are baked) | n/a | Do not document as API-driven; waypoint data is manually maintained |
...
| S17 Price Performance | `src/features/modules/live/S17_PricePerformance.jsx` | `fetchBtcSpot()` + `/api/s17/house-price` | BTC spot: Binance -> Binance.US -> cached `btcRates`; FRED MSPUS quarterly series via `server/features/fred/s17HousePrice.js` | BTC spot on load; FRED data quarterly (cached by backend) | Backend serves stale cached FRED+Binance payload; frontend keeps previous chart state | `src/shared/services/priceApi.js`, `server/services/btcRates.js`, `server/features/fred/s17HousePrice.js`, `server/app.js`, `src/features/modules/live/S17_PricePerformance.jsx` |
...
| S23 Big Mac Index | `src/features/modules/under-construction/S23_BigMacIndex.jsx` | `fetchBtcSpot()` | Binance -> Binance.US -> cached `btcRates` | On load | Local fallback price `84000` in component after shared spot helper returns null | `src/shared/services/priceApi.js`, `server/services/btcRates.js`, `src/features/modules/under-construction/S23_BigMacIndex.jsx` |
