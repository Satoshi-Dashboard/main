...
function MempoolChip({ block, idx, chipW }) {
  ...
  return (
    ...
    >
      <span className="font-mono text-white/50" style={{ fontSize: Math.max(11, Math.round(9 * sc)) }}>
        {idx === 0 ? 'NEXT' : `+${idx}`}
      </span>
      {/* fill bar */}
      ...
      <span className="font-mono text-white/50" style={{ fontSize: Math.max(11, Math.round(8 * sc)) }}>
  ...
  return (
    <div className={`flex flex-col gap-1.5 rounded-lg bg-[#0d0d0d]/95 p-3 ${className}`} style={{ border: `1px solid ${UI_COLORS.brand}4d` }}>
      <div className="flex justify-between items-center mb-0.5">
        ...
        <button type="button" onClick={onClose} className="inline-flex min-h-[32px] min-w-[32px] items-center justify-center rounded-full text-sm text-white/50 transition hover:bg-white/6 hover:text-white/70">✕</button>
      </div>
      <div className="border-t border-[#1c1c1c]" />
      {rows.map(([label, val]) => (
        <div key={label} className="flex justify-between items-baseline gap-2">
          <span className="text-[12px] font-mono text-white/50 uppercase tracking-wide flex-shrink-0">{label}</span>
    ...
    <div className="flex flex-wrap items-center gap-2">
      {FEE_SCALE.map(({ color, label }) => (
        <div key={label} className="flex items-center gap-1">
          ...
          <span className="text-[12px] font-mono text-white/50">{label}</span>
        </div>
      ))}
...
export default function S05_LongTermTrend() {
  ...
  const { data, error: fetchError } = useModuleData(fetchLiveData, {
    ...
    bootstrapKey: 'mempoolLive',
    // Redis guarda { data: { blocks: [...], mempool_blocks: [...], fees: {...} }, ... }
    // fetchLiveData() devuelve { blocks, mempoolBlocks, fees, lastUpdatedAt }
    bootstrapTransform: (raw) => {
      const d = raw?.data;
      if (!d) return null;
      // Use feeRange from first pending mempool block (real decimal fees).
      // fees/recommended floors at 1 sat/vB by policy — only use as fallback.
      const mq = Array.isArray(d.mempool_blocks) ? d.mempool_blocks : [];
      const fr = Array.isArray(mq[0]?.feeRange) ? mq[0].feeRange : [];
      const rec = d.fees || {};
      const firstFin = (...vals) => { for (const v of vals) { const n = Number(v); if (Number.isFinite(n)) return n; } return null; };
      return {
        blocks: Array.isArray(d.blocks) ? d.blocks.slice(0, 8) : [],
        mempoolBlocks: mq.slice(0, 8),
        fees: {
          economy:  firstFin(fr[1],            rec.economyFee),   // p25
          halfHour: firstFin(fr[4],            rec.halfHourFee),  // p90
          fastest:  firstFin(fr[fr.length - 2], rec.fastestFee),  // p95
        },
        lastUpdatedAt: raw.updated_at || new Date().toISOString(),
      };
    },
  });
  ...
  return (
    <ModuleShell bg="#111111" overflow="hidden" className="select-none font-mono">
      ...
      {/* ── Fee tiles: Low Priority / Medium Priority / High Priority ── */}
      <div className="flex flex-shrink-0 divide-x divide-[#2a2a2a] border-b border-[#1c1c1c]">
        {[
          { label: 'Low Priority',  val: fees?.economy,  col: '#00FFCC' },
          { label: 'Medium Priority', val: fees?.halfHour, col: '#FF8C00' },
          { label: 'High Priority', val: fees?.fastest,  col: '#FF6B6B' },
        ].map(({ label, val, col }) => (
          <div key={label} className="flex flex-1 flex-col items-center gap-0.5 px-2 py-2 text-center">
            ...
            <span className="font-mono uppercase tracking-[0.18em] text-white/50" style={{ fontSize: 'var(--fs-tag)' }}>{label}</span>
