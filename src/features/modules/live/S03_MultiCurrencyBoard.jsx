...
export default function S03_MultiCurrencyBoard() {
  ...
  const { data: priceData, loading: isPriceLoading } = useModuleData(fetchCurrencies, {
    ...
    bootstrapKey: 's03MultiCurrency',
    transform: transformCurrencies,
    // /api/s03/multi-currency retorna estructura PLANA: { rates:{EUR,GBP,...}, changes_24h_pct:{EUR,...}, source_provider, is_fallback, ... }
    // NO tiene clave "data" — los datos están en el nivel raíz del objeto.
    // fetchMultiCurrencyBtc() devuelve { usd: 84000, eur: 70000, usd_24h_change: 1.5, __source, ... }
    // transformCurrencies() necesita ese shape → se aplica directamente aquí
    bootstrapTransform: (raw) => {
      ...
      for (const [key, val] of Object.entries(rates)) {
        ...
        if (Number.isFinite(price) && price > 0) btcObj[code] = price;
      }
      const changes = raw?.changes_24h_pct || {};
      for (const [key, val] of Object.entries(changes)) {
        const change = Number(val);
        if (Number.isFinite(change)) btcObj[`${key.toLowerCase()}_24h_change`] = change;
      }
      btcObj.__source = raw?.source_provider || '';
      btcObj.__is_fallback = Boolean(raw?.is_fallback);

      if (Object.keys(btcObj).length < 2) return null;
      // Aplica transformCurrencies para obtener el shape final { currencies, sourceLabel, lastUpdatedAt }
      return transformCurrencies(btcObj);
    },
  });
  ...
  const filtered = currencies.filter(
    c =>
      c.code.toLowerCase().includes(deferredSearch) ||
      String(c.name || '').toLowerCase().includes(deferredSearch)
  );
  ...
  return (
    <ModuleShell overflow="hidden">
      ...
      <div className="min-h-0 flex flex-col lg:flex-1 lg:flex-row">
        ...
        {/* Right panel */}
        <div className="visual-integrity-lock flex h-[min(40vh,360px)] min-h-[280px] min-w-0 flex-none flex-col border-t border-[#1a1a1a] bg-[#111111] sm:h-[min(42vh,420px)] sm:min-h-[320px] lg:h-auto lg:min-h-0 lg:w-[clamp(180px,22vw,260px)] lg:border-l lg:border-t-0">
              ...
              return (
                <div key={c.code} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
                    ...
                    <span className="text-safe-wrap" style={{
