...
const ChartSection = memo(function ChartSection({ points, onHoverChange }) {
  ...
  const labelMap = useMemo(() => {
    const map = new Map();
    points.forEach((point) => map.set(Math.floor(point.ts / 1000), point.date ?? null));
    return map;
  }, [points]);
  ...
  useEffect(() => {
    ...
    const onTouchMove = (event) => {
      ...
      const dx = Math.abs(event.touches[0].clientX - touchX0);
      const dy = Math.abs(event.touches[0].clientY - touchY0);
      ...
      onHoverChange({
        usd: usdBar.value,
        homeInBtc: btcBar?.value ?? null,
        date: labelMap.get(typeof usdBar.time === 'number' ? usdBar.time : null) ?? null,
      });
    };
  ...
  useEffect(() => {
    ...
    if (!usdSeries || !btcSeries || !chart || !points.length) return;

    const usdData = points
      .filter((point) => Number.isFinite(point.usd) && Number.isFinite(point.ts))
      .map((point) => ({ time: Math.floor(point.ts / 1000), value: point.usd }));
    const btcData = points
      .filter((point) => Number.isFinite(point.homeInBtc) && Number.isFinite(point.ts))
      .map((point) => ({ time: Math.floor(point.ts / 1000), value: point.homeInBtc }));
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'flex-end', gap: 2, padding: '8px 0' }}>
      {Array.from({ length: 48 }, (_, index) => (
        <div key={index} style={{ flex: 1, borderRadius: 2, height: `${22 + Math.sin(index * 0.35) * 16 + Math.sin(index * 0.12) * 24}%`, background: 'rgba(255,255,255,0.06)', animation: `s17-pulse 1.6s ease-in-out ${index * 30}ms infinite` }} />
      ))}
    </div>
  );
}
...
export default function S17_PricePerformance() {
  const fetchSpot = useCallback(() => fetchBtcSpot().then((spot) => spot?.usd ?? null), []);
  ...
  const displayUsd = hoveredPoint?.usd ?? payload?.data?.latest_value ?? null;
  const displayBtc = hoverData ? hoveredPoint?.homeInBtc : (liveHomeInBtc ?? hoveredPoint?.homeInBtc ?? null);

  const firstPoint = points[0] ?? null;
  const usdDeltaPct = firstPoint && latestPoint ? ((latestPoint.usd - firstPoint.usd) / firstPoint.usd) * 100 : null;
  const btcDeltaPct = firstPoint && latestPoint ? ((latestPoint.homeInBtc - firstPoint.homeInBtc) / firstPoint.homeInBtc) * 100 : null;

  const fredQuarter = payload?.data?.quarter_label ?? null;
  const handleHover = useCallback((point) => setHoverData(point), []);
