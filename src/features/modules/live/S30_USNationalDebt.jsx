...
function StatCard({ label, value, variant = 'number', helper, accent = 'var(--text-primary)', featured = false, className = '', animate = true }) {
  return (
    <SurfaceCard
      className={`p-3.5 text-left sm:p-4 lg:p-5 2xl:p-6 ${className}`}
    >
      <div
        className="text-safe-wrap font-mono uppercase"
        style={{
      ...
      {helper ? (
        <div
          className="text-safe-wrap mt-3 font-mono leading-relaxed"
...
function RateCard({ label, value, toneColor, animate = true }) {
  return (
    <SurfaceCard tone="muted" className="p-3.5 text-left sm:p-4 xl:p-5 2xl:p-4">
...
function ErrorState({ message, onRetry }) {
  return (
    <div className="mx-auto flex min-h-full w-full max-w-xl items-center justify-center py-8">
      <SurfaceCard className="w-full px-6 py-8 text-center sm:px-8">
        ...
        <div className="text-safe-wrap mt-2 font-mono leading-relaxed" style={{ color: 'var(--text-secondary)', fontSize: 'var(--fs-body)' }}>
...
export default function S30_USNationalDebt() {
  ...
  if (loading && !model) {
    return (
      <ModuleShell layout="none" className="relative overflow-y-auto">
  ...
  return (
    <ModuleShell layout="none" className="relative overflow-y-auto lg:overflow-hidden">
      ...
      <div className="relative mx-auto flex min-h-full w-full max-w-[1720px] flex-col px-4 py-4 sm:px-5 sm:py-5 lg:h-full lg:px-10 lg:py-6 xl:px-12 2xl:px-16 2xl:py-7">
        <div className="mx-auto flex w-full max-w-[1520px] flex-col items-center gap-4 text-center sm:gap-5 lg:h-full lg:flex-1 lg:justify-between lg:gap-0">
          <header className="flex w-full max-w-[980px] flex-col items-center gap-4 max-md:gap-3">
            ...
            <div
              className="flex max-w-[26rem] flex-wrap items-center justify-center gap-x-2 gap-y-1 font-mono sm:max-w-full"
              style={{ color: 'var(--text-secondary)', fontSize: 'var(--fs-micro)' }}
            >
          ...
          <section className="flex w-full max-w-[980px] flex-col items-center gap-3">
            ...
            <div
              className="flex min-h-[2.75rem] max-w-full flex-wrap items-center justify-center gap-x-2 gap-y-1 text-safe-wrap font-mono tabular-nums"
          ...
          {(payload?.is_fallback || payload?.fallback_note || error) ? (
            <div
              className="text-safe-wrap mt-5 w-full rounded-2xl border px-3 py-2 font-mono text-left"
              style={{
