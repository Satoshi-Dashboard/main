...
export default function S16_MayerMultiple() {
  ...
  const { data: historyData, loading, error: historyError } = useModuleData(fetchHistory, {
    ...
    bootstrapKey: 'binanceHistory2025',
    transform: (raw) => (raw?.length ? raw : []),
    // Redis guarda { data: [{ ts, price }, ...], updated_at, ... }
    // fetchBtcHistory() devuelve [{ ts, price, axisLabel, tooltipLabel }]
    // calcularMayerMultiple() solo necesita { ts, price } → extraemos el array interno
    bootstrapTransform: (raw) => {
      const rows = Array.isArray(raw?.data) ? raw.data : null;
  ...
  const { data: liveSpot } = useModuleData(fetchSpot, {
    ...
    // fetchSpot() devuelve un número (usd price)
    // Redis guarda { btc_usd: 84000, ... }
    bootstrapTransform: (raw) => {
