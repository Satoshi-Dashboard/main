...
export default function S07_LightningNodesMap() {
  ...
  // Individual node network data — points from nodes/world, lines from channels-geo
  ...
  // Per-capita maps for all 3 metrics (values per million inhabitants)
  const perCapitaByCode = useMemo(() => {
    ...
    metricRows.forEach((row) => {
      ...
      map[c].capacity += row.capacity / pop / 100_000_000; // stored as BTC/M
    });
    return map;
  }, [metricRows, populationMap]);

  // Max absolute per metric
  ...
  // Avg channel geographic distance per node pubkey (km)
