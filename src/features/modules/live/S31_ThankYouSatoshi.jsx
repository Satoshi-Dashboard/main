...
import { SurfaceCard } from '@/shared/components/layout/AppLayout.jsx';
...
const THANK_YOU_MESSAGES = [
  ...
  'Dhanyavad Satoshi Nakamoto',
  'Dhonnobad Satoshi Nakamoto',
  'Shukriya Satoshi Nakamoto',
  'Spasibo Satoshi Nakamoto',
  'Diakuiu Satoshi Nakamoto',
  'Efcharisto Satoshi Nakamoto',
  'Toda Satoshi Nakamoto',
  'Mamnoon Satoshi Nakamoto',
  'Xiexie Satoshi Nakamoto',
  'Arigatou Satoshi Nakamoto',
  'Gamsahamnida Satoshi Nakamoto',
...
function CardTitle({ children, accentDot = false }) {
  return (
    <div className="flex items-center gap-2 font-mono text-white" style={{ fontSize: 'var(--fs-section)' }}>
      {accentDot ? <span style={{ color: 'var(--accent-bitcoin)' }}>•</span> : null}
      <span className="text-safe-wrap">{children}</span>
    </div>
  );
}
...
export default function S31_ThankYouSatoshi() {
  ...
  const onCopyAddress = async () => {
    const copiedAddress = await copyTextToClipboard(DONATION_ADDRESS);
  ...
  return (
    <ModuleShell layout="none" className="flex items-start justify-center overflow-y-auto lg:items-center">
      <div className="w-full max-w-[1240px] px-5 py-6 pb-20 sm:px-9 sm:py-8 sm:pb-24 lg:px-14 lg:py-8 lg:pb-16 xl:px-20 xl:py-10">
        <button
          type="button"
        ...
        >
          ...
          <div className="mx-auto mt-3 flex h-[clamp(68px,8vw,108px)] max-w-[1040px] items-center justify-center overflow-hidden px-2">
            <span
              className="text-safe-wrap font-semibold leading-tight"
              style={{
        ...
        <div className="grid gap-3.5 sm:gap-4 lg:grid-cols-2">
          <SurfaceCard tone="muted" className="p-4 sm:p-5">
            <CardTitle accentDot>Genesis Block Information</CardTitle>
            ...
            <div className="mt-3.5 space-y-1.5 font-mono" style={{ fontSize: 'var(--fs-body)' }}>
              ...
              <div className="text-safe-wrap text-white">{GENESIS_HASH}</div>
              <div style={{ color: 'var(--text-secondary)' }}>Block Height 0 - January 3, 2009</div>
            </div>
          </SurfaceCard>

          <SurfaceCard tone="muted" className="p-4 sm:p-5">
            <CardTitle>Bitcoin Principles</CardTitle>
            <div className="mt-3 h-px bg-white/10" />
            <ul className="mt-3.5 space-y-2 font-mono" style={{ fontSize: 'var(--fs-body)' }}>
              {PRINCIPLES.map((item) => (
                <li key={item} className="flex items-start gap-2.5 text-safe-wrap" style={{ color: 'var(--text-secondary)' }}>
                  <span
          ...
          <SurfaceCard tone="muted" className="p-4 sm:p-5">
            <div
              className="text-safe-wrap font-mono italic leading-relaxed text-white"
          ...
          <SurfaceCard tone="muted" className="p-4 sm:p-5">
            <CardTitle>Support this Dashboard</CardTitle>
            <p
              className="mt-1.5 text-safe-wrap font-mono"
            ...
            <div className="mt-3.5 flex flex-col gap-2.5 sm:flex-row sm:items-start">
              <button
                ...
                title="Click to copy"
              >
                <span className="text-safe-wrap block pr-5 leading-4">{DONATION_ADDRESS}</span>
                <span
                  className="absolute inset-0 hidden items-center justify-center bg-[#111111]/90 opacity-0 transition-opacity group-hover/addr:opacity-100 md:flex"
            ...
            <div className="mt-2.5 flex flex-col gap-2">
              ...
              >
                <Github size={14} />
                <span className="text-safe-wrap">github.com/Satoshi-Dashboard</span>
              </a>
              <a
              ...
              >
                <Github size={14} />
                <span className="text-safe-wrap">Project supporters</span>
              </a>
            </div>
          </SurfaceCard>
        </div>
