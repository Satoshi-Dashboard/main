...
export default function S18_CycleSpiral() {
  ...
  useEffect(() => {
    ...
    const handleWindowMouseMove = (event) => {
      setPan({
        x: initialPan.x + (event.clientX - dragStart.x),
        y: initialPan.y + (event.clientY - dragStart.y),
      });
    };
  ...
  }, [dragStart, initialPan, isDragging]);

  const todayIndicator = useMemo(() => {
    const currentPrice = latestPrice || waypoints.at(-1)?.price || null;
    if (!currentPrice || currentPrice <= 0) return null;

    const todayYear = getFractionalYear(todayTimestamp);
    ...
    const { x, y } = polarToCartesian(angle, todayRadius, dims.CX, dims.CY);

    return {
      x,
      y,
      dateLabel: new Date(todayTimestamp).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
    };
  }, [dims.CX, dims.CY, dims.R_MAX, dims.R_MIN, latestPrice, todayTimestamp, waypoints]);
            ...
            >
              ...
              {todayIndicator && (
                <g key="today-indicator">
                  ...
                  <text
                    x={todayIndicator.x}
                    y={todayIndicator.y - (containerWidth < 640 ? 10 : 12)}
                    textAnchor="middle"
                    fill="white"
                    opacity="0.8"
                  ...
                  <text
                    x={todayIndicator.x}
                    y={todayIndicator.y + (containerWidth < 640 ? 12 : 14)}
                    textAnchor="middle"
                    fill="white"
                    opacity="0.6"
