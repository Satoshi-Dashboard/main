 ...
 *
 * @param {string} interval - Candlestick interval (default: '1d' for daily)
 ...
 */
export function useBinanceHistoricalBTC(interval = '1d', refreshMs = 300000) {
  // Fetch raw klines from Binance API
  const { data: klines, loading, error, refetch } = useModuleData(
    useCallback(async () => {
      try {
        const klinesData = await fetchBTCHistoricalKlines(interval);
        return klinesData;
      } catch (err) {
        console.error('[useBinanceHistoricalBTC] Error fetching klines:', err);
        throw err;
      }
    }, [interval]),
    {
