...
export function resolveMempoolOverviewBundle(overviewPayload, livePayload) {
  ...
  return {
    ...
    fees: {
      economy: firstFiniteNumber(feeSnapshot.hourFee, lastBlock?.feeRange?.[0]),
    ...
    is_fallback: Boolean(overviewPayload?.is_fallback),
    source_provider: typeof overviewPayload?.source_provider === 'string' ? overviewPayload.source_provider : null,
  };
}
...
export function resolveMempoolOfficialUsageSnapshot(payload) {
  ...
  return {
    ...
    source_provider: typeof payload?.source_provider === 'string' ? payload.source_provider : null,
  };
}
