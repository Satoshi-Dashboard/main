...
export function resolveMempoolOverviewBundle(overviewPayload) {
  ...
  return {
    ...
    fees: {
      economy:  toFiniteNumber(feeSnapshot.economyFee),
      normal:   toFiniteNumber(feeSnapshot.halfHourFee),
      priority: toFiniteNumber(feeSnapshot.fastestFee),
    },
  };
}
...
export async function fetchMempoolOverviewBundle(options = {}) {
  ...
  _inflightBundle = (async () => {
    const overviewPayload = await fetchJson('/api/public/mempool/overview', { timeout, cache });
    return resolveMempoolOverviewBundle(overviewPayload);
  })();
