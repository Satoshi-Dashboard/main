...
- `server/.runtime-cache/`: local runtime fallback files for backend caches and other persisted operational snapshots.
...
11. Do not write runtime cache or fallback data to the repository root; persist it via `server/core/runtimeCache.js` or `server/.runtime-cache/`.
...
- **Archivo(s) Afectado(s):** `.claude/repo/PROJECT_STRUCTURE.md`
- **Tipo de Evento/Contexto:** Persistencia runtime fuera de la raiz del repo
- **Descripción del Evento Original:** Una ruta de fallback de Lightning guardaba snapshots operativos fuera del area runtime, mezclando datos mutables con la raiz del workspace.
- **Acción Realizada/Corrección:** Se consolidó `server/.runtime-cache/` como ubicación explícita para snapshots runtime y se añadió una regla estructural para impedir nuevos caches persistidos en la raíz.
- **Nueva/Modificada Regla o Directriz:** Toda persistencia runtime local debe vivir en `server/.runtime-cache/` o a través de `server/core/runtimeCache.js`; la raíz del repo queda reservada para código y configuración versionada.
- **Justificación:** Evita ruido en git, reduce la mezcla entre código y artefactos operativos, y hace más segura la gestión de datos temporales del backend.
