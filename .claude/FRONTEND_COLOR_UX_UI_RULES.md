...
- S04 Mempool Gauge
- S05 Long-Term Trend
- S06 BTC Queue (`S06_BtcQueue.jsx`)
- S07 Nodes Map (`S06_NodesMap.jsx`)
- S08 Lightning Nodes Map (`S07_LightningNodesMap.jsx`)
- S09 BTC Map Business Density (`S08_BtcMapBusinessesMap.jsx`)
- S10 Lightning Network (`S09_LightningNetwork.jsx`)
- S11 Stablecoin Peg Health (`S10_StablecoinPegHealth.jsx`)
...
- **Archivo(s) Afectado(s):** `.claude/FRONTEND_COLOR_UX_UI_RULES.md`
- **Tipo de Evento/Contexto:** Nuevo modulo de fee bands con referencia visual a S02 y reindexacion del registry
- **Descripcion del Evento Original:** Al insertar un nuevo modulo live despues de `S05`, la lista base de los primeros 11 modulos quedo desactualizada y el nuevo chart de BTC Queue exponia 58 buckets raw del upstream, demasiado densos para una UI nativa del dashboard.
- **Accion Realizada/Correccion:** Se actualizo la lista base de modulos y se consolido el patron para agrupar buckets raw en bandas visibles mas legibles cuando el upstream entregue demasiadas series para una lectura humana razonable.
- **Nueva/Modificada Regla o Directriz:** Si una visualizacion live hereda decenas de buckets raw desde un proveedor externo, frontend debe agruparlos en bandas legibles y explicitar ese agrupamiento en la UI en lugar de volcar todas las series sin jerarquia ni legibilidad movil.
- **Justificacion:** Mantiene honestidad de datos sin sacrificar UX, evita tooltips/legendas imposibles de escanear y ayuda a que nuevos charts densos sigan pareciendo piezas nativas del producto.
