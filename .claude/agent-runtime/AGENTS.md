...
24. **Rule 24 (No Public Mutation Surface):** Any new or existing `POST`, `PUT`, `PATCH`, or `DELETE` endpoint must be treated as private by default. Public mutation routes require explicit owner approval, auth, rate limiting, and a documented reason.
25. **Rule 25 (No Blanket Proxy Trust):** Never set `trust proxy` to blanket `true`. Trust only explicit proxy hops, known subnet presets, or platform-specific settings such as Vercel's single hop.
26. **Rule 26 (No Hardcoded Production Backends):** Do not hardcode production service URLs, project refs, or secrets as runtime fallbacks in source code. If a backend dependency is required, fail closed and require explicit environment configuration.
...
- **Fecha de la Actualización:** `2026-03-26`
- **Archivo(s) Afectado(s):** `.claude/agent-runtime/AGENTS.md`
- **Tipo de Evento/Contexto:** Hardening de endpoints privados, proxy trust y backends sensibles
- **Descripción del Evento Original:** La auditoría detectó un endpoint de escritura accesible bajo una ruta `public`, una configuración `trust proxy` demasiado amplia y un fallback hardcodeado hacia un backend Supabase de producción.
- **Acción Realizada/Corrección:** Se añadieron reglas runtime para prohibir superficies de mutación públicas por defecto, impedir `trust proxy: true` sin restricciones y vetar fallbacks hardcodeados a infraestructuras productivas.
- **Nueva/Modificada Regla o Directriz:** **Rules 24-26** exigen auth/rate limiting para mutaciones, confianza de proxy explícita y configuración de backends sensibles exclusivamente por variables de entorno.
- **Justificación:** Reduce riesgo de bypass de auth, abuso de rutas administrativas y acoplamiento accidental a infraestructura real al clonar o desplegar el proyecto.
