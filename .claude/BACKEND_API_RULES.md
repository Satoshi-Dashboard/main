...
6. Any `POST`, `PUT`, `PATCH`, or `DELETE` route must be private by default; only expose public mutation routes with explicit owner approval plus auth, rate limiting, and a documented justification.
7. Do not place private/admin behavior under misleading `public` semantics in code or route naming unless backward compatibility forces the path; in that case, keep the guardrails and document the exception.
8. Never set Express `trust proxy` to blanket `true`; trust only explicit hop counts, subnet presets, or platform-specific known values.
9. Do not hardcode production backend URLs, project refs, or tokens as runtime fallbacks. Sensitive integrations must fail closed when required env vars are missing.
10. In ESM files, do not use `require(...)` for runtime helpers or cache persistence.
11. Runtime cache artifacts must use `server/core/runtimeCache.js` or live under `server/.runtime-cache/`, never the repo root.
...
- **Descripción del Evento Original:** Las reglas backend/API no incluían un anexo histórico obligatorio para registrar correcciones y mejoras.
- **Acción Realizada/Corrección:** Se creó el registro histórico al final para documentar lecciones futuras.
- **Nueva/Modificada Regla o Directriz:** Todo cambio backend/API debe registrar en este archivo cualquier ajuste relevante de reglas o conocimiento.
...
- **Fecha de la Actualizacion:** `2026-03-26`
- **Archivo(s) Afectado(s):** `.claude/BACKEND_API_RULES.md`
- **Tipo de Evento/Contexto:** Hardening de mutaciones privadas, proxy trust y cache runtime
- **Descripcion del Evento Original:** La auditoria encontro una ruta de escritura expuesta, una configuracion de proxy demasiado permisiva, uso de `require(...)` dentro de ESM y un cache fallback escribiendo fuera del area runtime controlada.
- **Accion Realizada/Correccion:** Se reforzo la politica de seguridad para exigir mutaciones privadas por defecto, prohibir `trust proxy: true`, vetar fallbacks productivos hardcodeados y obligar a que caches/runtime helpers usen `server/core/runtimeCache.js` o `server/.runtime-cache/`.
- **Nueva/Modificada Regla o Directriz:** Las reglas de seguridad 6-11 fijan auth/rate limiting para mutaciones, proxy trust acotado, envs explicitos para backends sensibles y persistencia runtime fuera de la raiz del repo.
- **Justificacion:** Convierte en checklist repetible los fallos detectados en la auditoria y reduce la probabilidad de reintroducir rutas administrativas expuestas o acoplamientos inseguros.
