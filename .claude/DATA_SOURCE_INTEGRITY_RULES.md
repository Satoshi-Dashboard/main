...
| S06 BTC Queue | Johoe-style BTC mempool queue history with stacked fee bands | Internal Supabase Edge Functions backed by Supabase Postgres queue tables for history (`24h`, `30d`, `all`) and latest snapshot, normalized into grouped fee bands by the backend; legacy ZatoBox/Johoe scraper remains emergency fallback only | Latest about every 1 min; 24h history about every 1 min; 30d history about every 15 min; all-history about every 6h | Use stale raw and composed backend payloads, fall back to the latest history point if the latest snapshot is temporarily unavailable, and only use the legacy scraper path when internal Supabase fetches fail | Do not expose Supabase service credentials to the browser, bypass `/api/*`, or fabricate fee-band history from unofficial proxies |
| S07 Nodes Map | Bitcoin nodes world map | Bitnodes data, preferably through `api.zatobox.io` scraper output; otherwise direct Bitnodes API; if that fails, scrape Bitnodes HTML modal | UI polls about every 10 min; backend follows Bitnodes snapshot/fallback windows | HTML scrape fallback or stale payload | Do not replace Bitnodes with another node provider silently |
| S08 Lightning Nodes Map | Lightning nodes world distribution | mempool.space lightning world data + Natural Earth geography | About every 60s | Use stale cached payload | Do not replace mempool.space or Natural Earth silently |
| S09 BTC Map Business Density | Merchant/business map by country | BTC Map places API plus Natural Earth country geometry/matching | UI about every 10 min; backend aggregate about every 6h | Use stale cached aggregate | Do not replace BTC Map silently |
| S10 Lightning Network | Lightning stats panel using BTC price anchor | BTC price from Binance | About every 15s in UI, underlying spot about every 5s | Keep previous UI value or cached backend value | Do not replace Binance silently |
| S11 Stablecoin Peg Health | Stablecoin peg monitor | CoinGecko stablecoin list, live prices, and per-coin details | Usually every 2 min; detail payload can live longer | Use stale list/live/detail payloads | Do not replace CoinGecko silently |
| S12 Fear & Greed | Fear & Greed module | Alternative.me | About every 6h | Use stale cached payload | Do not replace Alternative.me silently |
| S13 Address Distribution | BTC address distribution | BitInfoCharts, preferably via scraper proxy, otherwise direct shared scrape/parser | About every 30 min | Use stale cached payload | Do not replace BitInfoCharts silently |
| S14 Wealth Pyramid | Richness tiers / wealth pyramid | BitInfoCharts, preferably via scraper proxy, otherwise direct shared scrape/parser | About every 30 min | Use stale cached payload | Do not replace BitInfoCharts silently |
| S15 Global Assets | BTC vs global asset values | Newhedge snapshot, preferably via `api.zatobox.io`, otherwise fetched through the `r.jina.ai` mirror | About every 60 min | Use stale cached snapshot | Do not replace Newhedge silently |
| S16 BTC vs Gold | BTC vs gold comparison | BTC price/history from Binance with backend-derived BTC market cap from protocol issuance estimates; gold current market cap from `api.zatobox.io/api/scrape/companiesmarketcap-gold` | BTC history about every 5 min; gold snapshot about every 15 min | Backend serves cached/stale Binance+Zatobox comparison payload | Do not replace Binance or Zatobox/CompaniesMarketCap silently |
| S17 Mayer Multiple | Live Mayer Multiple chart | BTC spot from Binance plus shared daily BTC history from Binance/Binance.US; Mayer Multiple is derived client-side from that history with a 200-day SMA warmup | Spot about every 10s in UI; daily history requested on load and effectively cached at the long-range backend cadence | Use frontend memory cache, backend stale history payload, and keep the last good spot value | Do not swap Binance spot/history silently |
| S18 Price Performance | US Median Home Price in BTC | BTC spot from Binance; U.S. median home price time series from FRED MSPUS via `/api/s17/house-price` backend endpoint | Quarterly FRED data on load; BTC spot on load | Backend serves cached FRED+Binance comparison payload; stale cache on failure | Do not replace FRED or Binance silently |
| S19 Cycle Spiral | Live interactive halving-cycle spiral | Local handcrafted halving cycle waypoint data; fully interactive SVG with tooltips, phase legend, responsive scaling | Static (waypoints are baked) | n/a | Do not document as API-driven; waypoint data is manually maintained |
| S20 Power Law Model | Under-construction model page | Local model data | Static | n/a | Do not document as live API data |
| S21 Stock to Flow | Under-construction model page | Local model data | Static | n/a | Do not document as live API data |
| S22 Big Mac Sats Tracker | Under-construction purchasing-power tracker | BTC spot from Binance, Big Mac price from The Economist CSV, historical BTC closes from Binance/Binance.US | UI about every 5 min; combined backend feed is slow-moving and cached | Use stale combined cache | Do not replace Economist/Binance silently |
| S23 Seasonality | Under-construction heatmap | Local static dataset | Static | n/a | Do not document as live API data |
| S24 Big Mac Index | Under-construction quick burger/BTC comparison | BTC spot from Binance; burger price is local/static in the component | On load | Component falls back to a baked BTC value if needed | Do not document as fully live multi-source data |
| S25 Network Activity | Under-construction activity panel | Local synthetic/mock dataset | Static | n/a | Do not document as live API data |
| S26 Log Regression | Under-construction model page | Local model data | Static | n/a | Do not document as live API data |
| S27 MVRV Score | Under-construction model page | Local model data | Static | n/a | Do not document as live API data |
| S28 Google Trends | Under-construction trends page | Local synthetic/mock dataset | Static | n/a | Do not document as live API data |
| S29 BTC Dominance | Under-construction dominance page | Local dataset | Static | n/a | Do not document as live API data |
| S30 UTXO Distribution | Under-construction UTXO page | Local static dataset | Static | n/a | Do not document as live API data |
| S31 U.S. National Debt | Treasury debt module | U.S. Treasury FiscalData plus latest available U.S. Census ACS population estimate | Debt series about every 15 min; population estimate about every 30 days; UI interpolates every second | Use stale cached debt/population payloads and keep local interpolation running | Do not replace Treasury/Census silently |
| S32 Thank You Satoshi | Closing tribute module | Local static content only | Static | n/a | Do not document as API-driven |
...
| S06 BTC Queue | `src/features/modules/live/S06_BtcQueue.jsx` | `fetchBtcQueuePayload()` + `/api/public/mempool/btc-queue` | `SUPABASE_PROJECT_URL/functions/v1/johoe-btc-queue-history` + `SUPABASE_PROJECT_URL/functions/v1/johoe-btc-queue-latest` invoked server-to-server with `SUPABASE_FUNCTIONS_JWT`/`SUPABASE_ANON_KEY`; Edge Functions read `johoe_queue_24h_rolling` with `SUPABASE_SERVICE_ROLE_KEY`; legacy `SCRAPER_BASE_URL/api/scrape/johoe-btc-queue/*` is allowed only as 24h freshness fallback when Supabase is stale or unavailable | Latest 1 min; history 24h 1 min | Shared stale raw feeds plus stale composed payload; if Supabase latest lags the backend falls back to the last history point, and if Supabase 24h is stale the backend may use the legacy scraper for 24h only | `server/services/publicDataFeeds.js`, `server/app.js`, `src/shared/services/btcQueueApi.js`, `src/features/modules/live/S06_BtcQueue.jsx`, `supabase/functions/johoe-btc-queue-history/index.ts`, `supabase/functions/johoe-btc-queue-latest/index.ts` |
| S07 Nodes Map | `src/features/modules/live/S06_NodesMap.jsx` | `/api/bitnodes/cache` | `SCRAPER_BASE_URL/api/scrape/bitnodes-nodes` API data -> direct Bitnodes API + snapshot -> Bitnodes HTML modal scrape fallback | Backend follows Bitnodes next snapshot / scrape throttle; UI poll ~10 min | Existing fallback payload or stale cache | `server/features/bitnodes/bitnodesCache.js`, `server/app.js` |
| S08 Lightning Nodes Map | `src/features/modules/live/S07_LightningNodesMap.jsx` | `/api/public/lightning/world` | mempool.space lightning world | 60s | Shared stale payload in `getFeed()` | `server/services/publicDataFeeds.js` |
| S09 BTC Map Business Density | `src/features/modules/live/S08_BtcMapBusinessesMap.jsx` | `/api/public/btcmap/businesses-by-country` | BTC Map places API + Natural Earth high-res countries | Backend 6h; UI poll ~10 min | Shared stale aggregate payload | `server/services/publicDataFeeds.js` |
| S10 Lightning Network | `src/features/modules/live/S09_LightningNetwork.jsx` | `fetchBtcSpot()` | Binance -> Binance.US -> cached `btcRates` | Spot 5s; UI 15s | Cached `btcRates` payload / previous UI state | `src/shared/services/priceApi.js`, `server/services/btcRates.js` |
| S11 Stablecoin Peg Health | `src/features/modules/live/S10_StablecoinPegHealth.jsx` | `/api/s10/stablecoins`, `/api/s10/stablecoins/live-prices`, `/api/s10/stablecoin/:id` | CoinGecko markets list -> derived live-price payload -> CoinGecko detail endpoint | List/live 2 min; detail cache 10 min TTL with slower effective reuse | Stale list/live/detail payloads from memory/KV | `server/features/stablecoins/s10StablecoinPegCache.js`, `server/app.js` |
| S12 Fear & Greed | `src/features/modules/live/S11_FearGreedIndex.jsx` | `/api/public/fear-greed?limit=31` | Alternative.me | 6h | Shared stale payload in `getFeed()` | `server/services/publicDataFeeds.js` |
| S13 Address Distribution | `src/features/modules/live/S12_AddressDistribution.jsx` | `/api/s12/btc-distribution` | `SCRAPER_BASE_URL/api/scrape/bitinfocharts-richlist` -> direct BitInfoCharts HTML shared parser | 30 min | Stale shared payload | `server/services/bitinfochartsShared.js`, `server/features/bitinfocharts/s12BtcDistribution.js`, `server/app.js` |
| S14 Wealth Pyramid | `src/features/modules/live/S13_WealthPyramid.jsx` | `/api/s13/addresses-richer` | `SCRAPER_BASE_URL/api/scrape/bitinfocharts-richlist` -> direct BitInfoCharts HTML shared parser | 30 min | Stale shared payload | `server/services/bitinfochartsShared.js`, `server/features/bitinfocharts/s13AddressesRicher.js`, `server/app.js` |
| S15 Global Assets | `src/features/modules/live/S14_GlobalAssetsTreemap.jsx` | `/api/s14/global-assets` | `SCRAPER_BASE_URL/api/scrape/newhedge-global-assets` -> `https://r.jina.ai/http://newhedge.io/bitcoin/global-asset-values` | 60 min | Stale snapshot from KV/memory | `server/features/global-assets/s14GlobalAssetsCache.js`, `server/app.js` |
| S16 BTC vs Gold | `src/features/modules/live/S15_BTCvsGold.jsx` | `/api/s15/btc-vs-gold-market-cap` | Binance/Binance.US daily BTC history + `/api/btc/rates` spot -> backend BTC market-cap derivation from protocol issuance schedule; gold: `SCRAPER_BASE_URL/api/scrape/companiesmarketcap-gold` | BTC history 5 min; BTC spot 5s; gold snapshot 15 min | Backend payload built from stale/cached Binance history and stale/cached Zatobox gold snapshot | `server/services/publicDataFeeds.js`, `server/services/btcRates.js`, `server/app.js`, `src/features/modules/live/S15_BTCvsGold.jsx` |
| S17 Mayer Multiple | `src/features/modules/live/S16_MayerMultiple.jsx` | `fetchBtcSpot()` + `fetchBtcHistory(2025, '1d')` | Spot: Binance -> Binance.US -> cached `btcRates`; history: Binance -> Binance.US | Spot poll 10s in UI; warmup-adjusted long-range daily history requested on load with effective 60 min backend cadence | Frontend memory cache in `priceApi.js`, backend stale history payload, previous UI spot value | `src/shared/services/priceApi.js`, `src/shared/utils/mayerMultiple.js`, `src/features/modules/live/S16_MayerMultiple.jsx` |
| S18 Price Performance | `src/features/modules/live/S17_PricePerformance.jsx` | `fetchBtcSpot()` + `/api/s17/house-price` | BTC spot: Binance -> Binance.US -> cached `btcRates`; FRED MSPUS quarterly series via `server/features/fred/s17HousePrice.js` | BTC spot on load; FRED data quarterly (cached by backend) | Backend serves stale cached FRED+Binance payload; frontend keeps previous chart state | `src/shared/services/priceApi.js`, `server/services/btcRates.js`, `server/features/fred/s17HousePrice.js`, `server/app.js`, `src/features/modules/live/S17_PricePerformance.jsx` |
| S22 Big Mac Sats Tracker | `src/features/modules/under-construction/S21_BigMacSatsTracker.jsx` | `/api/public/s21/big-mac-sats-data` | `/api/btc/rates` spot + Economist CSV + Binance/Binance.US historical close lookups | Combined feed 7d; subfeeds 12h-24h; UI 5 min | Shared stale combined payload | `server/services/publicDataFeeds.js`, `server/services/btcRates.js`, `src/features/modules/under-construction/S21_BigMacSatsTracker.jsx` |
| S24 Big Mac Index | `src/features/modules/under-construction/S23_BigMacIndex.jsx` | `fetchBtcSpot()` | Binance -> Binance.US -> cached `btcRates` | On load | Local fallback price `84000` in component after shared spot helper returns null | `src/shared/services/priceApi.js`, `server/services/btcRates.js`, `src/features/modules/under-construction/S23_BigMacIndex.jsx` |
| S31 U.S. National Debt | `src/features/modules/live/S30_USNationalDebt.jsx` | `/api/public/us-national-debt` | Treasury Debt to the Penny -> latest available ACS 1-Year population year | Debt 15 min; population 30d; UI local 1s interpolation loop | Combined stale payload from debt/population feeds | `server/services/publicDataFeeds.js`, `src/shared/services/usNationalDebtApi.js`, `server/app.js` |
...
- **Fecha de la Actualización:** `2026-03-29`
- **Archivo(s) Afectado(s):** `.claude/DATA_SOURCE_INTEGRITY_RULES.md`
- **Tipo de Evento/Contexto:** Simplificación operativa de S06 BTC Queue a 24h only
- **Descripción del Evento Original:** El módulo BTC Queue mantenía referencias activas a `30d` y `all`, pero la base real solo conservaba `johoe_queue_24h_rolling` y la falta de ingesta fresca en Supabase estaba dejando el módulo congelado mientras esas rutas fantasma añadían errores y ruido operativo.
- **Acción Realizada/Corrección:** Se fijó la política del módulo a `24h` solamente, se retiró la semántica operativa de `30d/all` y se permitió usar el scraper legacy solo como fallback de frescura para `24h` cuando Supabase esté stale o caído.
- **Nueva/Modificada Regla o Directriz:** S06 BTC Queue debe operar únicamente sobre la ventana `24h`; no se deben reintroducir rangos `30d` o `all` sin recrear explícitamente toda la cadena de almacenamiento, lectura y observabilidad.
- **Justificación:** Reduce superficie rota, alinea código con el esquema real y evita que un writer externo caído vuelva a quedar oculto tras rangos no soportados.

- **Fecha de la Actualizacion:** `2026-03-25`
- **Archivo(s) Afectado(s):** `.claude/DATA_SOURCE_INTEGRITY_RULES.md`
- **Tipo de Evento/Contexto:** Migracion interna de S06 BTC Queue hacia Supabase Edge Functions
- **Descripcion del Evento Original:** El frontend habia empezado a apuntar directo a `functions/v1`, pero el repo exige que el navegador consuma `/api/*` y que los secretos/server-side no se expongan al cliente.
- **Accion Realizada/Correccion:** Se actualizo la politica de fuentes para fijar Supabase Edge Functions + Supabase Postgres como backend interno de `S06`, manteniendo `/api/public/mempool/btc-queue` como unica superficie publica del modulo y dejando el scraper legacy como fallback de contingencia.
- **Nueva/Modificada Regla o Directriz:** En `S06 BTC Queue`, cualquier integracion con Supabase debe quedar detras de `/api/*`, invocada server-to-server con secretos server-side; el navegador no debe llamar directo a `functions/v1`.
- **Justificacion:** Reduce superficie publica, preserva la arquitectura del repo, evita exponer credenciales y mantiene cache/fallback/rate limiting centralizados.

- **Fecha de la Actualización:** `2026-03-09`
- **Archivo(s) Afectado(s):** `.claude/DATA_SOURCE_INTEGRITY_RULES.md`
- **Tipo de Evento/Contexto:** Configuración de automejora
- **Descripción del Evento Original:** La política de integridad de fuentes no incluía un registro histórico obligatorio para cambios derivados de fallos o mejoras detectadas.
- **Acción Realizada/Corrección:** Se añadió la sección histórica para registrar futuras correcciones relacionadas con proveedores, fallback y cadencias.
...
- **Archivo(s) Afectado(s):** `.claude/DATA_SOURCE_INTEGRITY_RULES.md`
- **Tipo de Evento/Contexto:** Alineacion skills-first y consolidacion de consumidor BTC compartido
- **Descripcion del Evento Original:** Tras adoptar skills tecnicas como base y limpiar duplicacion en previews S17/S23, la politica de fuentes seguia describiendo consumidores directos de `/api/btc/rates` en lugar del helper compartido de spot BTC.
...
- **Archivo(s) Afectado(s):** `.claude/DATA_SOURCE_INTEGRITY_RULES.md`
- **Tipo de Evento/Contexto:** Nuevo modulo S06 BTC Queue con Johoe/ZatoBox y reindexacion total del registry
- **Descripcion del Evento Original:** Se inserto un nuevo modulo live despues de `S05`, cambiando la numeracion de todos los modulos posteriores, y el producto paso a consumir un nuevo flujo aprobado de ZatoBox para historia/bandas del BTC queue con `latest` separado del historial.
- **Accion Realizada/Correccion:** Se añadieron las filas humana y tecnica de `S06 BTC Queue`, se documentaron sus endpoints aprobados, cadencias y fallback, y se renumeraron las filas posteriores para mantener coherencia con `src/features/module-registry/modules.js`.
- **Nueva/Modificada Regla o Directriz:** Cuando un nuevo modulo live reindexe el registry, esta politica debe actualizar en la misma tarea tanto el nuevo flujo de datos como los codigos de todos los modulos desplazados; no se permite dejar tablas historicas con numeracion desfasada respecto al registry vivo.
- **Justificacion:** Evita ambiguedad entre numero de modulo, slug y fuente aprobada, y reduce el riesgo de que futuros agentes conecten APIs correctas al modulo equivocado por confiar en tablas desactualizadas.
