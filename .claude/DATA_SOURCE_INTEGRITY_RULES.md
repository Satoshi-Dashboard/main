...
| S06 BTC Queue | `src/features/modules/live/S06_BtcQueue.jsx` | `fetchBtcQueuePayload(range)` + `/api/public/mempool/btc-queue?range=24h|30d|all` | `SUPABASE_PROJECT_URL/functions/v1/johoe-btc-queue-history?range=*` + `SUPABASE_PROJECT_URL/functions/v1/johoe-btc-queue-latest` invoked server-to-server with `SUPABASE_FUNCTIONS_JWT`/`SUPABASE_ANON_KEY`; Edge Functions read Postgres with `SUPABASE_SERVICE_ROLE_KEY`; legacy `SCRAPER_BASE_URL/api/scrape/johoe-btc-queue/*` remains fallback only | Latest 1 min; history 24h 1 min; history 30d 15 min; history all 6 h | Shared stale raw feeds plus stale composed payload; if Supabase latest lags the backend falls back to the last history point, and only falls back to the legacy scraper when the internal Supabase function fails | `server/services/publicDataFeeds.js`, `server/app.js`, `src/shared/services/btcQueueApi.js`, `src/features/modules/live/S06_BtcQueue.jsx`, `supabase/functions/johoe-btc-queue-history/index.ts`, `supabase/functions/johoe-btc-queue-latest/index.ts` |
